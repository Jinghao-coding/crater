...
| `cmd/root.go` 的 `Execute` | 通过 `signal.NotifyContext` 为 `rootCmd.ExecuteContext(ctx)` 注入 `os.Interrupt` 取消信号；执行结果非 nil 时调用 `handleError(err)`，再经 `exitCodeFor` 使用 `pkg/errorcodes.ExitCodeForCategory` 映射退出码并 `os.Exit`。 | `RunE` 成功返回后不再写 stdout；业务成功输出已在各命令内写完。 |
